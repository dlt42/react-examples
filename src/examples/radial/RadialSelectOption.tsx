...
const RadialSelectOption: FunctionComponent<RadialSelectOptionProps> = 
  memo(({
    label, onClickOption, startAngle, endAngle, diameter, 
    index, registerCallback, itemDiameter, centerDiameter 
  }): JSX.Element => {

  const [ optionState, setOptionState ] = useState<RadialSelectOptionState | null>(null);
  const ref = useRef<HTMLDivElement>(null);

  const stepCallback: StepCallback = useCallback((step: number) => {
    if (!optionState) 
      return;
    ...
    if (ref.current) {
      ref.current.style.top = ((Math.sin(degreesToRadians(angle)) * distance) + adj) + "px";
      ref.current.style.left = ((Math.cos(degreesToRadians(angle)) * distance) + adj) + "px";
  ...
  return (
    <div ref={ref} className="Radial-Option" onClick={(e: MouseEvent) => onClickOption(e, index)}>
      {label}
    </div>
  )
});
